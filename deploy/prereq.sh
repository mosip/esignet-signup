  ...
  while true; do
    read -p "Do you want to continue configuring Captcha secrets for signup ? (y/n) : " ans
      if [ $ans='Y' ] || [ $ans='y' ]; then
        ...
        ENV_VAR_EXISTS=$(kubectl -n captcha get deployment captcha -o jsonpath="{.spec.template.spec.containers[0].env[?(@.name=='MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_SINGUP')].name}")

        if [[ -z "$ENV_VAR_EXISTS" ]]; then
            # If the environment variable does not exist, add it
            echo "Environment variable 'MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_SINGUP' does not exist. Adding it..."
            kubectl patch deployment -n captcha captcha --type='json' -p='[{"op": "add", "path": "/spec/template/spec/containers/0/env/-", "value": {"name": "MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_SINGUP", "valueFrom": {"secretKeyRef": {"name": "signup-captcha", "key": "signup-captcha-secret-key"}}}}]'
        else
            # If the environment variable exists, update it
            echo "Environment variable 'MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_SINGUP' exists. Updating it..."
            kubectl patch deployment -n captcha captcha --type='json' -p='[{"op": "replace", "path": "/spec/template/spec/containers/0/env[?(@.name==\"MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_SINGUP\")]", "value": {"name": "MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_SINGUP", "valueFrom": {"secretKeyRef": {"name": "signup-captcha", "key": "signup-captcha-secret-key"}}}}]'
        fi
        break
