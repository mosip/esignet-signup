...
function installing_uitestrig() {
 ...
 if [[ $eSignetDeployed == "yes" ]]; then
  ...
  read -p "Is values.yaml for uitestrig chart set correctly as part of pre-requisites? (Y/n) : " yn;
  if [[ $yn = "Y" ]] || [[ $yn = "y" ]] ; then
    NFS_OPTION=''
    S3_OPTION=''
    ...
    while [ "$config_complete" = false ]; do
      ...
      if [[ "$ans" == "y" || "$ans" == "Y" ]]; then
        ...
        S3_OPTION="--set uitestrig.configmaps.s3.s3-host=$s3_host --set uitestrig.configmaps.s3.s3-user-key=$s3_user_key --set uitestrig.configmaps.s3.s3-region=$s3_region"
