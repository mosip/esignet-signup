...
public class SignUpSettingsControllerTest {
    ...
    public void getSignupSettings_thenPass () throws Exception {
        mockMvc.perform(get("/settings")
                ...
                .andExpect(jsonPath("$['response']['configs']['identifier.pattern']").value("\\d{8,9}"))
                .andExpect(jsonPath("$['response']['configs']['identifier.prefix']").value(""))
                .andExpect(jsonPath("$['response']['configs']['captcha.site.key']").value(""))
                .andExpect(jsonPath("$['response']['configs']['otp.length']").value(6))
                .andExpect(jsonPath("$['response']['configs']['password.pattern']").value(""))
                ...
                .andExpect(jsonPath("$['response']['configs']['resend.delay']").value(30))
                .andExpect(jsonPath("$['response']['configs']['fullname.pattern']").value(""))
                .andExpect(jsonPath("$['response']['configs']['status.request.delay']").value(10))
